...
if [ "${USE_DATA}" -eq 1 ]; then
  ...
  CMSDRIVER_COMMON_CMD+=" --filein root://eoscms.cern.ch//eos/cms/store/relval/CMSSW_12_5_0_pre5/RelValTTbar_14TeV/GEN-SIM-DIGI-RAW/PU_125X_mcRun3_2022_realistic_v3-v2/10000/314f66c0-136e-45c2-87ba-51f496879df1.root"
fi
...
#process.consumer.eventProducts = ['particleFlowRecHitHBHEOnly']
...
###
${CMSDRIVER_COMMON_CMD} --python_filename .tmp.py --customise_commands="process.particleFlowClusterHBHEOnly.cpu.pfClusterBuilder.maxIterations=5"
...
###
${CMSDRIVER_COMMON_CMD} --python_filename .tmp.py --procModifiers gpu --customise_commands="process.particleFlowClusterHBHEOnly.cpu.pfClusterBuilder.maxIterations=5"
...
if [ -f "${BENCHMARK_EXE}" ]; then
  ...
  #for cfgname in hcalOnGPU_pfhbheOnGPU; do
