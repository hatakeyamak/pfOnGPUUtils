...
{ 
  ...
  TString canvName = "PFCluster_GPUvsCPU_";
  canvName += W;
  canvName += "-";
  canvName += H;
  canvName += "_";  
...

  {
    ...
    TH1F *data = (TH1F*) (file_.Get("DQMData/Run 1/ParticleFlow/Run summary/pfClusterHBHEGPUHLTv/pfCluster_Energy_GPUvsCPU_")->Clone());
    data->SetDirectory(0);
    ...
    data->GetXaxis()->SetTitle("PF Cluster Energy (GeV) from CPU");
    data->GetYaxis()->SetTitle("PF Cluster Energy (GeV) from GPU");
    data->GetZaxis()->SetTitle("Number of PF Clusters");
    data->SetMaximum(100000000.);
    data->Print();
